...
function App() {
  ...
  const refreshBalance = useCallback(async (address) => {
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'Balance failed');
      setWallets((prev) => prev.map((w) => w.address === address ? { ...w, balance: data.sol, balanceError: '' } : w));
    } catch (error) {
      setWallets((prev) => prev.map((w) => w.address === address ? { ...w, balanceError: error.message } : w));
    }
  }, []);
  ...
  async function addWallet() {
    ...
    const data = await response.json();
    if (!response.ok) {
      setNotice(data.error || 'Invalid wallet address.');
      return;
    }
    const wallet = { id: crypto.randomUUID(), address, active: true, amount: '', balance: data.sol };
  ...
  async function validateMint() {
    ...
    try {
      const response = await fetch(`/api/mint?mint=${encodeURIComponent(value)}`);
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Invalid mint');
      setMintState({ status: 'valid', ...data });
    } catch (error) {
      setMintState({ status: 'error', message: error.message });
    }
  }
    ...
    await Promise.all(activeWallets.map(async (wallet) => {
      ...
      try {
        const response = await fetch('/api/order', {
          ...
          body: JSON.stringify({ wallet: wallet.address, outputMint: mint.trim(), amountSol: amount }),
        });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || 'Could not prepare order');
        next[wallet.address] = { status: 'ready', amount, ...data };
      } catch (error) {
        next[wallet.address] = { status: 'error', amount, error: error.message };
      }
      setOrders((prev) => ({ ...prev, [wallet.address]: next[wallet.address] }));
    }));
    setBusy(false);
  ...
  async function signAndExecute(address) {
    ...
    try {
      ...
      const response = await fetch('/api/execute', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ signedTransaction: signedBase64, requestId: order.requestId, provider: order.provider }),
      });
      const data = await response.json();
      if (!response.ok || data.status !== 'Success') throw new Error(data.error || `Swap failed (${data.code ?? 'unknown'})`);
      setOrders((prev) => ({ ...prev, [address]: { ...prev[address], status: 'success', signature: data.signature, result: data } }));
      refreshBalance(address);
    } catch (error) {
      setOrders((prev) => ({ ...prev, [address]: { ...prev[address], status: 'error', error: error.message } }));
    }
  }
      ...
      <section className="shell">
        ...
        <section className="section">
          ...
          <div className="field-row">
            ...
            <button className="text-button" onClick={validateMint}>Validate</button>
          </div>
